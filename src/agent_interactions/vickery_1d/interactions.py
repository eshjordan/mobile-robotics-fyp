...
def interact_1(agent1: Agent, agent2: Agent):

    """
    Algorithm 4
    """

    agent1.update_knowledge(max(agent1.n, agent2.n))
    agent2.update_knowledge(max(agent1.n, agent2.n))

    # agent1=agent2=N  will always be true
    N = agent1.n 
    agent2.move_to(agent1.theta_c() - 2*pi/N)


def interact_2(agent1: Agent, agent2: Agent):
    ...
    # agent1=agent2=N  will always be true
    N = agent1.n 
    agent1.move_to(agent2.theta_c() - 2*pi/N)


def test_neighbourhood(agent1: Agent, agent2: Agent):
    overlapping = (compute_overlap(agent1, agent2)>0 or
                   angles_equal(agent1.theta_l,agent2.theta_u) or
                   angles_equal(agent1.theta_u,agent2.theta_l))
    return overlapping